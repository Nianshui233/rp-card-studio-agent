...
const AUDIENCE_VISIBLE_INTERNAL_KEYS = new Set([
  "status",
  "scene_id",
  "scene_ref",
  "region_id",
...
export function validateUiExperienceSources({
  ...
  for (const binding of selectedBindings) {
    ...
    if (AUDIENCE_VISIBLE_INTERNAL_KEYS.has(visibleLabel))
      warnings.push(
        issue(
  ...
  const sections = (component.content.sections ?? [])
    .map((section) => {
      const sectionBody = (section.body ?? [])
        .map((paragraph) => `<p class="rp-copy">${escapeHtml(paragraph)}</p>`)
        .join("");
      return `<details class="rp-card rp-section"${section.collapsed ? "" : " open"}><summary>${escapeHtml(section.title)}</summary>${sectionBody}</details>`;
    })
    .join("");
...
function runtimeSource(component, componentBindings, theme, experience) {
  ...
  const hardenedRuntime = generatedRuntime
    ...
    .replace(
      "const render=()=>{try{hideState();wireLocal();wirePrimary();if(config.preset==='form_setup')renderForm();",
