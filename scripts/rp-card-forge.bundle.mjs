...
// scripts/ui-app-builder.mjs
var import_yaml = __toESM(require_dist(), 1);
import { createHash } from "node:crypto";
import { mkdir, readFile, writeFile } from "node:fs/promises";
import path from "node:path";
var STYLE_SLOT = "<!-- RP_UI_STYLES -->";
var SCRIPT_SLOT = "<!-- RP_UI_SCRIPTS -->";
var FRAGMENT_PATTERN = /<!--\s*RP_UI_FRAGMENT:([A-Za-z0-9_.-]+)\s*-->/g;
...
function parseManifestText(text, file) {
  ...
  return import_yaml.default.parse(text);
}
async function loadUiAppManifest(manifestPath) {
  ...
  if (document.schema_version !== void 0 && document.schema_version !== "1.0.0") {
...
function candidatePath(target, transactionId) {
  return path2.join(path2.dirname(target), `.${path2.basename(target)}.${transactionId}.candidate`);
}
function backupPath(target, transactionId) {
  return path2.join(path2.dirname(target), `.${path2.basename(target)}.${transactionId}.backup`);
}
...
async function commitNewDirectory(root, files, options = {}) {
  ...
  const candidateRoot = path2.join(
    path2.dirname(absoluteRoot),
    `.${path2.basename(absoluteRoot)}.${process.pid}-${randomUUID()}.candidate`
  );
  await mkdir2(path2.dirname(absoluteRoot), { recursive: true });
  await mkdir2(candidateRoot, { recursive: false });
  try {
...
function resolveWithin(root, relativePath) {
  ...
  const absoluteRoot = path2.resolve(root);
  const resolved = path2.resolve(absoluteRoot, relativePath);
  const relative = path2.relative(absoluteRoot, resolved);
...
async function withProjectLock(projectRoot, callback, { force = false, dryRun = false } = {}) {
  ...
  const lockPath = path2.join(projectRoot, ".rp-card-forge.lock");
...
function nonCardChunkDigest(chunks) {
  return sha2562(Buffer.concat(chunks.filter((chunk) => !isCardChunk(chunk)).map((chunk) => chunk.raw)));
}
...
function parseYamlText(text, label = "YAML") {
  const document = import_yaml2.default.parseDocument(text, {
    prettyErrors: true,
    strict: true,
    uniqueKeys: true
  });
...
async function readYaml(path52) {
  return parseYamlText(await readUtf8(path52), path52);
}
function stringifyYaml(value) {
  return import_yaml2.default.stringify(value, {
    indent: 2,
    lineWidth: 0,
...
function machineId(name) {
  ...
  return ascii || `project_${sha2562(name).slice(0, 10)}`;
}
...
async function projectFilesForInit(root, { type = "character", nsfw, stageRoute = DEFAULT_STAGE_ROUTE } = {}) {
  ...
  const userCharacter = type === "character" ? await readYaml(await templateAssetUrl("user-character.yaml")) : null;
...
async function commandRoundtrip(args, options) {
  ...
  return successReport("roundtrip", {
    ...
    payloadDigest: sha2562(prettyJson(comparable.payload)),
    candidateDigest: sha2562(candidate),
    png: pngEvidence,
    dryRun: Boolean(options["dry-run"])
  }, [], changes);
}
