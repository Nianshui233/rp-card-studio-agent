...
| 选择 | 适合什么项目 | 最低完整能力 |
| --- | --- | --- |
| `light` 轻型 | 单一核心玩法、信息不算少，但不需要大量工作台 | 至少 6 个组件；必须有入口、综合状态终端和至少一种剧情组件 |
| `medium` 中型 | 多系统、多角色或需要玩家创建与多个管理页面 | 至少 12 个组件；在轻型基础上增加创建页、至少 4 个状态/业务工作区和 2 类剧情组件 |
| `heavy` 重型 | 开放世界、长线经营、调查、生存、群像等复杂 RP 项目 | 至少 24 个组件；至少 8 个工作区、4 类剧情组件，并有中心页或混合导航 |

这里的轻型并不等于“几行 HTML 的小状态条”。它已经是一套完整的项目界面；中型和重型是在真实玩法职责上继续扩展。代码有多少行不是等级标准，复制空面板也不能凑数。技能会先从项目题材提炼颜色、字体、形状、纹理、动效和一个唯一视觉签名，避免每张卡都变成同一套黑底霓虹终端。
...
如果项目只有一个主要状态面板、入口说明和少量剧情组件，选轻型；需要玩家创建、任务/物品/关系等多个工作区时选中型；开放世界还要地图、势力、调查、经济、时间线、行程等长期管理能力时选重型。技能会根据已经锁定的玩法给出推荐。你若把 UI 阶段完全交给 AI，它会说明选择和理由后直接锁定，不再重复问。

无论选择哪一级，都不是按 HTML 行数报价。一个 5000 行但只有一个杂乱面板的页面，仍可能比六个职责清晰、可降级、可编辑重绘的轻型组件更差。
...
| UI 2.0 | 把完整配套界面拆成体验、主题、绑定和多个消息组件，再编译进单一角色卡 JSON 的结构 |
