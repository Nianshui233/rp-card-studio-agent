...
| --- | --- |
| 指定项目放在哪里 | 创建并维护项目工作区 |
| 选择是否启用 NSFW，并在首轮选择项目阶段路线 | 记录开关与 `workflow.selected_stages`，后续严格按路线执行 |
...
5. **项目阶段路线**：材料整理、系统、场景、MVU/EJS、状态栏/UI 分别进入还是跳过。
6. **附加交付物**：默认最终只交付一个角色卡 `.json`；PNG、独立世界书等只有你明确需要时才加入。
...
任务类型：新建
已有材料：无
阶段路线：跳过材料整理；进入系统、场景、MVU/EJS、状态栏/UI
交付物：一个角色卡 JSON（默认）
```
...
下表给出固定的阶段顺序，但哪些可选阶段实际进入，必须在首轮预检中选择并写入 `project.yaml.workflow.selected_stages`。后续只按这份记录执行，不能因为对话变长、模型忘记提醒或默认流程存在，就把已跳过阶段重新加入。续作、修改、转换和审查任务优先恢复已有路线与进度。
...
| 阶段 | 用通俗的话说 | 是否可跳过 |
| --- | --- | --- |
| 项目预检 | 确认工作区、NSFW、任务类型、材料、阶段路线和交付物，不讨论创作内容 | 否 |
...
| 叙事与开场 | 决定叙事合同、开场事实、未解决压力、钩子、用户交接和呈现变体 | 否 |
...
| 内容 | 放置位置 | 为什么 |
| --- | --- | --- |
| 项目核心入口合同 | `data.description`（卡面里的“角色描述”） | 大型项目常驻完整世界骨架、信息分层、核心冲突、规则裁决、硬/软设定与自主运行原则；可以很长，但不放任何角色档案 |
| 默认开场 | `data.first_mes`（卡面里的“开场白”） | 普通项目直接放开场；完整入口 HTML 项目可只放 `Create_character` 之类的稳定标记 |
| 备选开场 | `data.alternate_greetings` | 由 SillyTavern 的备选开场机制选择 |
| 世界设定、势力、规则 | CharacterBook 条目 | 可以按用途常驻或按关键词注入 |
| 固定角色/NPC | CharacterBook，通常每个角色整块放入 | 保持人格、动机、关系、知识和行为规则连续；只有确有不同触发需求时才拆分或分组 |
| 系统、场景、叙事规则、对白示例 | CharacterBook 条目 | 便于独立控制何时进入模型上下文 |
| MVU/EJS 合同、状态栏回复格式 | CharacterBook 条目 | 它们是模型输出/运行合同，不是角色卡“高级定义”备注 |

SillyTavern 会把 `description` 当作常驻提示块注入，所以应把每轮都不能丢的世界核心合同放在这里。“入口”指模型进入整个 RP 项目的常驻入口，不等于短简介。世界、玩法、群像和长期项目默认在整合阶段生成 `assembly.card_entry.mode: core_world_contract`；定位阶段的 `positioning.card_entry` 只作为方向种子与兼容回退。
...
新建卡的高级定义字段默认可以留空，但它们不是禁区。项目需要传统角色卡兼容、短常驻合同、作者备注、示例对白、系统提示或插件行为时，可通过 `assembly.card_fields` 显式写入。修改旧卡时继续保留已有值，除非显式覆盖或锁定迁移策略。

世界书装配不重新创作内容，也不把维护对象序列化成 JSON。前面阶段保持完整 YAML；最终整合时按运行职责切片并设置触发、插入、深度、顺序、概率和递归。世界观按共同触发主题切块；固定角色/NPC通常整块放入；完整系统和场景可整块按需触发。条目正文继续使用紧凑 YAML/自然语言。

## 世界观和角色源码不是配置表

从 `1.2.0` 开始，新建世界观和角色源码采用“自主运转”的 RP 作者模板。稳定英文 ID 和少量维护字段仍保留，用于引用、校验和重建；真正的正文则先回答“这个世界和这些人物在没有任何特定介入者时，怎样继续生活和变化”。`1.0.0`、`1.1.0` 旧源码仍兼容。

世界观、NPC 与普通场景不围绕用户角色建立，也不会因为没有用户角色介入就停摆：世界有自己的日常、势力行动、资源流和事件时钟，NPC 有自己的职责、关系、计划与离场行动。这个原则不适用于把合法的开场交接、自由创角或用户角色模板删掉。项目可以在开场提供预制视角、自由创角或两者并存，也可以在世界书中预留一份默认关闭的“用户角色定义模板”；该模板独立于世界、NPC 和场景，填写后再启用。

世界观模板 [world.yaml](assets/templates/world.yaml) 主要包含：

- 类型、基调与实际 RP 形式；
- 时空范围、地点作用和环境资源；
- 公开信息、条件信息、秘密、谣言和揭示规则；
- 冲突各方、升级路径、个人层面的两难与可能解法；
- 每套世界规则能做什么、不能做什么、代价、上限和结果裁决；
- 种族、势力、社会秩序、普通常识与日常描写锚点；
- 地区与机构的日常节奏、势力正在执行的行动、事件时钟、资源流、信息流和场外结算频率；
- AI 即兴 NPC 的能力上限、背景、命名、复用和多样性；
- 历史如何影响当前局势，以及哪些内容属于硬设定、软设定或可探索方向。

角色模板 [character.yaml](assets/templates/character.yaml) 主要包含：

- 能被场景稳定观察到的外貌、体态、服装和第一印象；
- 当前目标与驱动力、恐惧、价值排序、底线和内在冲突；
- 职责、日程、私人生活、可调动资源、真实依赖、当前计划、期限和离场后的下一步；
- 合作、冲突、道德两难和压力触发下的具体行为与代价；
- 对亲近者、陌生人、敌人、权威和特殊对象的不同态度；
- 语气、句式、词汇、称呼、禁用表达、情绪变化和校准台词；
- 只保留会影响当前行为的背景与转折；
- 关系变化依据、认知误区、秘密、成长压力和反 OOC 规则。

用户角色模板 [user-character.yaml](assets/templates/user-character.yaml) 是另一类独立源码：

- 它不属于世界观，也不属于 NPC 角色清单；
- 投影为中文命名的独立 CharacterBook 条目，默认关闭；
- 默认关键词为 `<user>` 与 `user`，位置为 `after_char`，深度 4，顺序 9995，概率 100%；
- 填写并启用后才参与提示词；保持空白和关闭时不影响世界/NPC继续运转；
- 字段可与“自定义角色创建页”对齐，项目可选择只填世界书、只用 UI、二者任选，或显式合并。
因此，模板中的每个重要描述都应尽量回答“平时本来在做什么、接下来会做什么、什么情况下改变安排、怎样行动和说话、为什么、会付出什么代价”。旧源码仍然可读，不会因为升级作者模板而强制迁移或破坏已有项目。
...
项目首轮只决定这些可选阶段“进入”还是“跳过”，不提前设计其内部细节。进入 MVU/EJS 或 UI 后，才询问该阶段自己的变量、实现或界面问题。
...
| 选择 | 适合什么项目 | 常见设计方向 |
| --- | --- | --- |
| `light` 轻型 | 成熟完整、表现克制 | 完整入口与状态应用、统一主题、成熟排版、必要数据和实用交互 |
| `light_medium` 轻中型 | 题材拟物、行动联动增强 | 像题材内真实工具，拥有更多直接行动、反馈与宿主输入/命令联动 |
| `medium` 中型 | 产品级伴随系统 | 丰富层次、语义化视图、搜索筛选、弹窗、快捷操作与连贯反馈 |
| `heavy` 重型 | 华丽、沉浸、强主题化 | 多层空间、复杂动效、可视化、趣味交互和深度宿主协作 |
| `super_heavy` 超重型 | 独立前端 / 0层游玩卡 | 消息 HTML 成为主要游玩应用，采用集中状态、路由/多视图、复杂弹窗、持久化、全屏和应用级生命周期 |

轻型不等于“几行 HTML 的小状态条”；轻中型、中型、重型也不靠固定页数、模块数或代码量判定。一个大型完整 HTML 可以承载重型体验，多个页面也可能只是轻型。超重型的分界不在长度，而在消息 UI 是否已经成为应用级主要游玩界面。技能按玩家需求、信息架构、视觉完成度、真实数据、交互和运行时深度来决定，并从项目题材提炼颜色、字体、形状、纹理、动效和视觉签名。

完整 UI 首先是 RP 伴随界面，不是后台 Dashboard 或变量查看器。首屏应说明当前地点、正在发生的变化和值得注意的人物或事件。一级入口通常宜保持精炼，但不是固定不超过 5 个；复杂项目可按实际任务模型增加。大量内部能力优先收纳到二级册签、分区、折叠和上下文入口，避免导航噪音。地点、人物、情报、事务和旅行使用各自的中文语义视图，技术 ID、英文变量键、未知枚举、空对象与 `uninitialized` 不会直接显示。
...
`basic_status` 默认使用 `adapter: sillytavern_regex`。完整的 `light`、`light_medium`、`medium`、`heavy`、`super_heavy` UI 则使用 `adapter: tavern_helper_message` 与 `level: host_required`。入口流程与综合状态终端默认各是一份完整 HTML 应用，包含 `DOCTYPE`、`html/head/body`、CSS、JavaScript、页内导航和数据绑定；任务、物品、关系、地图等共享触发与状态快照的功能优先成为状态应用内部模块。检定、选项、变量报告和通知因触发时机不同，才保留独立正则块。页面可以联动父页面或插件 DOM，但不能借此创建常驻状态栏/面板。真实宿主没有完成 iframe 导航、脚本执行和逐项验收前，一律保持 `runtime: not_run`。
...
### 轻型、轻中型、中型、重型、超重型 UI 到底怎么选？

档位不决定功能多少。同样一套入口、状态、任务、物品、关系、地图和创角功能，可以做成轻型到重型的不同表现：轻型已经是成熟完整前端，只是克制；轻中型加强题材拟物、直接行动和宿主联动；中型形成产品级伴随系统；重型追求华丽、炫酷、沉浸和复杂交互。超重型不是“再多一点特效”，而是独立前端 / 0层游玩卡：消息 HTML 成为主要游玩应用，使用集中状态、路由、多视图、复杂弹窗、持久化、全屏、观察器和深度宿主 API。技能会根据题材、审美、期望体验与可接受的维护成本给出推荐，而不是按模块数量判断。

无论选择哪一级，都不是按 HTML 行数报价。一个很长但不好看、不好用、不贴题的页面仍然不合格；反过来，轻型也不能以“简洁”为名删掉卡真正需要的功能。正确方向是功能由 RP 决定，档位决定表现有多华丽、顺手、有趣、沉浸，以及前端运行时有多深。超重型必须增加应用级生命周期与真实宿主验收，不能只把巨大源码当成完成。
...
| 独立前端 / 0层游玩卡 | `super_heavy` 的社区常用称呼：消息 HTML 成为主要游玩应用，聊天层主要承担叙事、存储与宿主通道 |
