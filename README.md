...
| 内容 | 放置位置 | 为什么 |
| --- | --- | --- |
| 项目级入口 | `data.description`（卡面里的“角色描述”） | 常驻说明这个 RP 项目提供什么体验、其余模块如何协作；不放任何角色档案 |
...
如果项目只有一个主要综合状态页、入口说明和少量剧情组件，选轻型；需要在综合页中管理任务、物品、关系、情报等多个模块时选中型；开放世界还要地图、势力、调查、经济、时间线、行程等高密度长期管理模块时选重型。技能会根据已经锁定的玩法给出推荐。你若把 UI 阶段完全交给 AI，它会说明选择和理由后直接锁定，不再重复问。
