...
| 选择 | 适合什么项目 | 最低完整能力 |
| --- | --- | --- |
| `light` 轻型 | 单一核心玩法、信息不算少，但不需要大量工作台 | 至少 3 个完整页面；综合工作区至少 4 个有数据的页内模块，并有至少一种剧情组件 |
| `medium` 中型 | 多系统、多角色或需要玩家创建与多个长期管理模块 | 至少 4 个完整页面；综合工作区至少 8 个页内模块，并有至少 2 类剧情组件 |
| `heavy` 重型 | 开放世界、长线经营、调查、生存、群像等复杂 RP 项目 | 至少 5 个完整页面；综合工作区至少 12 个页内模块、3 类剧情组件，并有中心页或混合导航 |

这里的轻型并不等于“几行 HTML 的小状态条”。它已经是一套完整的项目界面；中型和重型是在同一批完整页面中继续增加模块深度、信息密度、交互和长期管理能力。代码有多少行不是等级标准，复制空面板或把任务、物品、关系各拆成同壳正则也不能凑数。技能会先从项目题材提炼颜色、字体、形状、纹理、动效和一个唯一视觉签名，避免每张卡都变成同一套黑底霓虹终端。

完整 UI 首先是给玩家使用的 RP 伴随界面，不是后台 Dashboard 或变量查看器。首屏应说明“我在哪里、现在发生什么、谁或什么值得注意、下一步能做什么”。玩家一级入口通常不超过 5 个；中型/重型要求的 8/12 个内部模块会收纳到二级册签、分区、折叠和上下文入口中，不会直接变成十几个标签。地点、人物、情报、事务、旅行和玩家档案使用各自的中文语义视图，技术 ID、英文变量键、未知枚举、空对象与 `uninitialized` 不会直接显示。
...
状态/工作区 iframe 必须先等待 `Mvu`，再调用 Tavern Helper 的 `getAllVariables()`。该接口在消息 iframe 中按“全局→角色卡→聊天→截至当前楼的消息变量”合并，因此既能读取当前楼快照，也不会把页面绑定到最后一楼。消息脚本按可能重复执行设计：先清理上一份受管运行实例，静态按钮和标签用覆盖式绑定重新接管；不使用 `pagehide` 的 AbortSignal 清掉本地控件，因为 opening swipe 可能触发 `pagehide` 后仍继续使用当前 iframe。MVU 和宿主全局监听在实例替换或真正 `unload` 时移除。模型 block 负载由 Forge 生成的宿主正则捕获槽直接送入 iframe，不再调用 `getChatMessages()` 重读已替换正文。父页面/宿主 DOM 联动允许直接实现输入框预填、聚焦、用户确认后的发送和插件功能联动，无需先寻找替代 API；这类联动需要在目标版本实测，并清理注册到宿主的监听器和临时节点。自定义源码仍会拒绝危险动态 HTML 写入、凭据或私密存储访问、动态代码、失控的网络/远程 UI，以及用户源码中的 `状态/工作区 iframe 必须先等待 `Mvu`，再调用 Tavern Helper 的 `getAllVariables()`。该接口在消息 iframe 中按“全局→角色卡→聊天→截至当前楼的消息变量”合并，因此既能读取当前楼快照，也不会把页面绑定到最后一楼。静态按钮和标签只绑定一次；MVU 更新监听会在 `pagehide`/`unload` 时移除。模型 block 负载由 Forge 生成的宿主正则捕获槽直接送入 iframe，不再调用 `getChatMessages()` 重读已替换正文。父页面/宿主 DOM 联动允许直接实现输入框预填、聚焦、用户确认后的发送和插件功能联动，无需先寻找替代 API；这类联动需要在目标版本实测，并清理注册到宿主的监听器和临时节点。自定义源码仍会拒绝危险动态 HTML 写入、凭据或私密存储访问、动态代码、失控的网络/远程 UI，以及用户源码中的 `$&`、`$1` 等替换令牌；Forge 自己生成的受控 `$1` 捕获槽不在此列。没有真实宿主验收前，这项能力只能记录为规格或 `not_run`，不能写成 `embedded` 或 `runtime: pass`。`、`$1` 等替换令牌；同时要求 `&&`、`||` 后留空格，避免 `&&current` 之类源码被 HTML 命名实体解析破坏。Forge 自己生成的受控 `$1` 捕获槽不在此列，并会对最终替换脚本执行语法检查。没有真实宿主验收前，这项能力只能记录为规格或 `not_run`，不能写成 `embedded` 或 `runtime: pass`。
...
当前测试覆盖源码重建一致性、外部运行时边界、技能入口与阶段规则、世界书装配、运行时状态 Schema、初始化 profile、跨文件引用、状态机、媒体、开场变体、EJS、真实宿主接口契约、UI 轻中重能力门槛、主题/绑定/组件 Schema、稳定正则、消息生命周期、安全扫描、自包含交付、输出保护、CLI 生命周期和未知字段拆包重建保留。
