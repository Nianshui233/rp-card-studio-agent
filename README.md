...
| `medium` 中型 | 多系统、多角色、连续管理 | 综合工作区、任务/物品/关系/情报等多种语义视图 |
...
`basic_status` 默认使用 `adapter: sillytavern_regex`。完整的 `light`、`medium`、`heavy` UI 则使用 `adapter: tavern_helper_message` 与 `level: host_required`。入口流程与综合状态终端默认各是一份完整 HTML 应用，包含 `DOCTYPE`、`html/head/body`、CSS、JavaScript、页内导航和数据绑定；任务、物品、关系、地图等共享触发与状态快照的功能优先成为状态应用内部模块。检定、选项、变量报告和通知因触发时机不同，才保留独立正则块。页面可以联动父页面或插件 DOM，但不能借此创建常驻状态栏/面板。真实宿主没有完成 iframe 导航、脚本执行和逐项验收前，一律保持 `runtime: not_run`。
...
轻/中/重不决定功能多少。同样一套入口、状态、任务、物品、关系、地图和创角功能，都可以做成三个档位：轻型偏清爽克制，使用成熟排版、少量动效和实用交互；中型更精致，加入更丰富的层次、切换、筛选、搜索、弹窗、反馈和快捷操作；重型追求华丽、炫酷、沉浸与强主题化，允许复杂动效、趣味交互、可视化、深度 JavaScript 联动和宿主协作。技能会根据题材、审美与期望体验给出推荐，而不是按模块数量判断。

无论选择哪一级，都不是按 HTML 行数报价。一个 5000 行但不好看、不好用、不贴题的页面仍然不合格；反过来，轻型也不能以“简洁”为名删掉卡真正需要的功能。正确方向是功能由 RP 决定，档位只决定这些功能做得多华丽、多顺手、多有趣、多沉浸。
