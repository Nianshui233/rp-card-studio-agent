...
基础状态栏默认使用 `adapter: sillytavern_regex`。项目需要动态刷新、命令、tabs、条件缺失值、运行时错误/加载态、精确数值格式化或逐楼层历史时，可以把 `adapter: tavern_helper_message` 与 `level: host_required` 作为用户明确选择的高级候选；选择适配器本身不代表这些能力已经实现或可靠。这条路径仍由角色正则消费占位符，替换结果是完整、自包含的 fenced HTML，再由 Tavern Helper 尝试把它变成该消息自己的 iframe。HTML、CSS、脚本和错误文案都随卡交付，不访问父页面、不创建页面常驻面板，也不加载远程 UI。真实宿主没有完成 iframe 导航、脚本执行和逐项验收前，一律保持 `runtime: not_run`。
