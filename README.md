...
从 `1.1.0` 开始，新建世界观和角色源码重新采用 RP 作者模板。稳定英文 ID 和少量维护字段仍保留，用于引用、校验和重建；真正的正文则围绕“游玩模型需要怎样理解并执行这个世界/角色”展开。
...
因此，模板中的每个重要描述都应尽量回答“什么情况下，会做什么、怎样说、为什么、会付出什么代价”。`1.0.0` 旧源码仍然可读，不会因为升级作者模板而强制迁移或破坏已有项目。
