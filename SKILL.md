...
- When MVU is enabled, include two strict regexes that hide complete `<initvar>...</initvar>` blocks from prompt and display copies while preserving the raw message, a prompt-only update-block filter that keeps the newest two history depths by default, and display-only update-block hiding. Generate both `[不发送]界面占位符` and `[界面]状态栏`; close the default opening, alternate openings, later reply contract, and message status regex as one system.
- Treat each opening `<initvar>` as a complete replacement of the fallback `[initvar]`, never a merge. Use the `mvu_json_patch` five-operation protocol and let each status field select `stat_data` or `display_data` according to its semantics.
- Runtime acceptance must verify the primary Character Lore import/binding, scoped-regex authorization, Tavern Helper character-script enablement, Tavern Helper macro enablement, ST-Prompt-Template enablement when EJS is used, and an observed MVU start/initialization event. If MVU is observed not to start while Blob URL rendering is enabled, recommend disabling it, refreshing SillyTavern, and observing again; never infer failure or success from that switch alone, and never patch SillyTavern or Tavern Helper source code.
