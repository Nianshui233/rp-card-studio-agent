...
- A full `light` UI has at least six components including an entry, a `status_terminal`, and a narrative component. `medium` has at least twelve, including a setup surface, four dashboard/workspace surfaces, and two narrative components. `heavy` has at least twenty-four, including eight dashboard/workspace surfaces, four narrative components, and `hub` or `mixed` navigation. These are capability floors, not targets to pad.
- Maintain UI 2.0 as separate `ui_experience`, `ui_theme`, `ui_bindings`, and `ui_component` sources. Bind player-visible fields from semantic `source_path` values to explicit MVU `runtime_path` values; UI never invents variable meaning. Compile each component to a stable Chinese-named character regex and, when the model produces its marker, a dedicated Chinese-named CharacterBook output contract. The default final artifact remains one character-card `.json` containing all compiled UI assets.
- Use only the message iframe and documented Tavern Helper interfaces needed by the component, such as current-message reads, message-scoped variables, message creation, slash triggering, opening swipe changes, and MVU events. Validate the current message ID, read `stat_data` from that message, set display text through `textContent` or controlled DOM nodes, bind interactions once, and remove MVU listeners on unload. Block parent-page DOM access, browser storage, dynamic code evaluation, unsafe HTML sinks, inline event handlers, network requests, remote fonts/icons/assets, and SillyTavern replacement tokens such as `$&` or `$1` in inline UI sources unless a future contract explicitly and safely supports them.
- Every UI component must define loading, empty, error, degraded, narrow-screen, edit-time, and plain-text fallback behavior. Respect keyboard focus, color-independent meaning, reduced motion, component and total byte budgets, and large-collection strategies. Source line count is never a quality or level metric.
