...
5. the project stage route: keep the standard order, and explicitly choose which optional stages to enter or skip;
6. whether the user explicitly needs any deliverable in addition to the default single character-card `.json`.

Do not ask about premise, world rules, characters, systems, plot, or UI during preflight. If no workspace is specified, ask the user to choose one and stop; never choose a write location on the user's behalf. Record the selected route in `project.yaml.workflow.selected_stages` and `preflight.workflow_confirmed` before entering positioning. For a resumed project with a valid preflight record, verify the recorded route and resume the recorded current stage without repeating questions or falling back to the default route.
...
After the user selects the optional stages, persist the complete ordered route with `state <workspace> lock preflight.stage_route '["positioning", ... , "integration"]' --force`. This updates `workflow.selected_stages`, locks the semantic decision, and marks every unselected optional stage `skipped` in technical state. Never infer the route from `features`, source folders, or the canonical default list.
...
After preflight, use the already recorded `project.yaml.workflow.selected_stages` route. The following is only the canonical order and the route-selection menu, not an automatic fallback:
...
`MVU/EJS` is optional, and whether it is entered is already locked in `workflow.selected_stages` during preflight. Do not ask again at the preceding stage. If selected, enter it without asking internal design questions early; if omitted, keep both features `false`, preserve its recorded `skipped` state, create no disabled placeholders or pseudo-implementation, and proceed to `narrative_opening`. A resumed, converted, revised, or audited project that skips the stage for this run preserves existing features and artifacts; disabling or removing an existing implementation requires opening the stage and completing migration plus validation.

`status_ui` is optional, and whether it is entered is already locked in `workflow.selected_stages` during preflight. Do not ask again before entering or skipping it. Once the recorded route enters it, its first question batch must ask which UI scale the project needs: `light`, `light_medium`, `medium`, `heavy`, or `super_heavy`; what visual/theme direction it should express; how ornate, animated, convenient, playful, and host-integrated the complete HTML should feel; whether entry routes are preset openings, free creation, or both; and whether the user-character definition is maintained in the worldbook template, the creation UI, either route, or an explicit merge. The scale measures visual finish, thematic immersion, interaction richness, JavaScript sophistication, convenience, fun, and runtime depth inside the required complete HTML; it never determines how many features, pages, fields, tabs, or regexes the RP is allowed to have. Treat `light` as a mature complete frontend baseline, not a small or crude status strip. Use `light_medium` for stronger diegetic controls and host-linked actions without an application runtime. Reserve `super_heavy` for an application-class independent frontend, also known as a zero-layer play card: the message HTML becomes the primary play surface and may use bundled frameworks, centralized state, routing, modal systems, persistence, fullscreen or multi-view behavior, observers, resource management, and deep SillyTavern/Tavern Helper APIs while leaving the host itself unmodified. Mention `basic_status` only as a compatibility route for an old project or an explicit request for a simple read-only status strip. Explain the practical difference and recommend one from the already locked project scope. Do not ask UI questions in an earlier stage. If the user delegates the UI stage completely, choose these items, report the decisions and reasons, lock them, and continue without asking again.
...
- Treat light/light-medium/medium/heavy/super-heavy as a presentation and runtime continuum: visual richness, thematic styling, motion, interaction depth, JavaScript convenience, playful details, host linkage, and application depth. The RP itself determines necessary information and functions. Page counts, module counts, navigation counts, source length, Chinese localization, mobile widths, touch sizes, accessibility, offline fallback, visual signatures, and portability remain project-specific recommendations, never universal blockers.
