...
Keep user-character definition separate from world, NPC, and ordinary scene sources. When the project needs a fixed custom persona, register an optional src/user-character.yaml source and project it as an independent, Chinese-named CharacterBook template that is disabled by default, constant, fter_char, depth 4, order 9995, and enabled only after it is filled. A character-creation UI may use the same field vocabulary; support worldbook-only, UI-only, either, or explicit merge. The world and NPCs must still have their own routines, agendas, clocks, relationships, and off-screen motion without waiting for that profile.
...
`status_ui` is optional. Before entering it, ask only whether to enter or skip. Once entered, its first question batch must ask which UI scale the project needs: `light`, `medium`, or `heavy`; whether it needs a full project introduction page; whether it needs a character-creation/start-configuration page; whether entry routes are preset openings, free creation, or both; and whether the user-character definition is maintained in the worldbook template, the creation UI, either route, or an explicit merge. Mention `basic_status` only as a compatibility route for an old project or an explicit request for a simple read-only status strip. Explain the practical difference and recommend one from the already locked project scope. Do not ask UI questions in an earlier stage. If the user delegates the UI stage completely, choose these items, report the decisions and reasons, lock them, and continue without asking again.
...
- Never define an in-fiction user or player character anywhere in the skill. Do not author their identity, background, personality, abilities, relationships, behavior, actions, decisions, goals, knowledge, agency, entry method, or point of involvement. Do not move such definitions into positioning, scenes, openings, systems, MVU, UI, card fields, or examples under another name. The authored project consists of its world, authored characters, systems, scenes, presentation, and their own autonomous behavior.
