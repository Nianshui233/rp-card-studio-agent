...
- Author new world and character sources as RP execution documents, not database/config inventories. New sources use schema `1.1.0`; keep `1.0.0` readable for existing projects.
- A world source should tell the play model what actions are possible, what resists them, how society and NPCs respond, how conflicts escalate, how rules adjudicate outcomes, and when information may be revealed. Use `world_identity`, `setting_scope`, `information_layers`, `core_conflict`, `world_rules`, `factions_and_society`, `improvised_characters`, `common_knowledge`, `timeline`, and `change_boundaries` as an RP World Bible.
- A character source should make choices and portrayal predictable through `identity`, `story_role`, motivations, fears, ranked values, taboos, internal conflict, trigger-to-action behavior, relationship attitudes, speech rules, useful background, growth pressure, and `anti_ooc`. Prefer observable behavior over abstract adjective lists.
