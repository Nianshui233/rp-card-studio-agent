...
这些样本是经过源码合同核对的原创参考。最小样本用于隔离单条技术路线；`full-mvu-rp` 是完整组合制品样例。源码核对与离线测试不等于 SillyTavern 实际导入通过。
...
| `full-mvu-rp` | ✓ 通知 | ✓ 两个独立前端 | ✓ | ✓ | ✓ 固定/动态 | ✓ 每楼快照 | ✓ 完整包 | **完整组合合同＋后续制作主参考（真实宿主验收状态见样例 README）** |
