...
这是样本矩阵中的**主组合参考样本**，目标不是把互斥实现机械叠加，而是展示一套职责闭合、可维护的制品路线。当前离线检查通过；真实宿主导入/运行状态以本 README 末尾证据为准，不据此承诺“可直接导入游玩”。
...
纯静态 UI、非 MVU 消息快照、STPT `@@iframe` 等是**替代载体**，不与本样本的同一状态表面重复安装。这里覆盖的是功能类别与预期运行闭环，而不是声称当前制品已完成目标宿主的端到端验收；也不会在一个聊天里制造多套权威状态。
...
以下是源码核对与实现目标版本，不是实机通过声明。只有在这些版本组合上重新导入并完成验收，才能把对应制品标为 runtime pass：
...
在仓库根目录运行只读检查，再在本目录运行会重建交付 JSON 的构建器：

```powershell
# 不写入文件：运行全部样例检查、Regex 夹具和合同回归
npm test

# 可选：对照本机 SillyTavern Regex placement 源码（仍是源码检查，不是 UI 导入）
npm run check -- --host-root "D:\AI\SillyTavern"

# 以下构建命令会写回本样例的生成制品；源码修改后运行
node .\_build.mjs
```
