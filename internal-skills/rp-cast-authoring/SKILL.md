...
- 每个角色维护完整 canonical YAML。进入世界书时保持一个连贯完整条目，或沿完整 YAML 子树无损拆分；不得压成短人设摘要或为分块而拆碎人格。
...
用户每次回答后立即写出或修改实际人物内容，并至少用初见、冲突、脆弱、拒绝或无人介入中的一个短场景校准承重人格。用户授权 AI 决定时直接选择并说明理由。阶段结束时由主 Agent 在对话中总结；不生成阶段账本、交接表或决定锁。
