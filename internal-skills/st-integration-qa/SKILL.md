...
- 最终角色卡必须自包含：源码路径、`source_refs`、`replace_file`、`content_file` 和绝对文件路径只能存在于项目源码/报告，不能进入可导入 `.json` 的正文或扩展；HTML、EJS、正则替换内容和 Tavern Helper 脚本必须已经内嵌。
- 远程依赖可以使用，但必须在交付报告中单列宿主/网络依赖；本地相对引用、工作区文件名和维护源码路径不能用“运行时会自己找到”来替代内嵌。
- 只要项目存在变量、MVU 或状态根，必须在最终制品中核对实际 Tavern Helper 脚本节点和代码内容；仅有变量说明、loader 占位或维护源路径不算完成。
- 在不修改宿主本体的前提下执行或记录真实 SillyTavern 验收。

旧卡改造额外检查：清理旧用户档案后，`src/user-character.yaml` 与最终 CharacterBook 都必须存在独立的中文、默认禁用 `<user>` 空白模板；缺失时即使其他内容完整也必须阻断交付。
