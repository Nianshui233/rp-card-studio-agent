...
- 默认/备用开场；
- 开场/创角前端的空白输入、主动路线选择、预览、真实写入、读回、失败保留和 greeting 交接；
- CharacterBook 导入和绑定；
...
- 持续消息前端的当前楼层/Swipe 数据、按钮、变量写入、同面读回与反馈；
- 开场前端写入的初态是否被持续消息前端沿同一路径读取，且没有重复初始化；
- 编辑、Swipe、删除、加载更多、重载、切聊和重复挂载；
- 窄屏、长中文、软键盘；
- Loader、远程 Schema 和断网失败态；
- 控制台首个因果错误。

开场创角提交至少使用一个主动选择的非默认值，并从稳定档案与动态状态各自的真实存储面读回；持续消息 UI 的写入动作也从其权威存储面读回。输入框里出现文本不等于变量已改变。
