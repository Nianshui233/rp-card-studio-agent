...
- 用户主控模板：角色卡项目默认保留一个独立、默认禁用、以 `<user>` 为触发键的中文世界书条目；填写 `src/user-character.yaml` 后再启用，或由开局 UI 收集后写入同一条目；普通世界、NPC 和场景不能依赖该条目才能运转。若项目本体就是“用户作为系统核心/宿主/操作员”，系统层可有自己的主控定义，但要与普通世界和 NPC 的自运转事实分开；
