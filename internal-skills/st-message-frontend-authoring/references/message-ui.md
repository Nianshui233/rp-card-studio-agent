...
不要把“不要问组件清单”误解成“不要问用户想要什么风格”。用户定制偏好优先于 Agent 的通用 UI 默认。除非用户已提供充分的设计要求或明确授权自由决定，否则在视觉实现前逐项了解：

1. **主题气质/参考**：界面要让人联想到怎样的作品内物件或氛围？喜欢/不喜欢哪些参考作品、界面、颜色感觉或视觉语言？
2. **展示内容**：哪些信息对玩家最重要、需要常驻？哪些辅助信息可折叠、按需展开或在变化时提示？是否要展示氛围/叙事信息，而不只是数值？
3. **排版与阅读密度**：用户偏好紧凑仪表盘、分区卡片、档案页、叙事面板或其他形式？最重要信息希望如何突出？长文本/大量物品怎样浏览？
4. **交互感受**：玩家最常做什么？操作是填入输入框、展开详情、切换视图还是确认写入？成功、冲突、保存失败希望怎样反馈？

可先推荐两三个有明显差异的方向并说明各自影响，再让用户选或组合；不要把大量问题压成一次“你想要什么 UI”。每轮最多 3 个独立决策。技术实现由 Agent 负责，不问用户框架、CSS token、DOM、API、iframe 或断点。

实现后不仅验证可读/可操作，也要以实际项目内容确认主题、信息层级、密度和交互是否兑现用户选择。再用加载中、无数据、保存失败、窄屏、长文本、Swipe 变化中的相关情境做稳健性校准。
