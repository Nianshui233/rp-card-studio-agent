...
`src/user-character.yaml` / 世界书 `<user>` 条目是唯一用户角色档案源。它只有最小骨架，没有固定现代人物字段；Agent 应根据当前项目决定什么是稳定档案、什么会随 RP 改变。奇幻的血脉或天赋可以是静态档案，当前魔力值是动态状态；修仙的灵根与道途可以是静态档案，当前真元、境界进度和劫数是动态状态。

UI 高频显示字段可以在开场时从 `<user>` 镜像到项目自己的状态根，但镜像应登记为只读。完整静态模块不得复制进 MVU。开场创角使用同一项目合同：静态字段生成 `<user>` YAML，动态字段生成初始状态补丁，分别写入并读回。字段数量、嵌套层级和中文命名不设上限，只有路径断链和重复写入权会阻断。
