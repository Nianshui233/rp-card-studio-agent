...
最终交付需要的是一个可直接粘贴到正则“替换内容”的完整、自包含 HTML；正则配置与 HTML 文件分开交付，这只是部署格式，不是开发方式。
...
最终项目包中，前端与正则分开交付：每个前端页面必须是一个完整、自包含的 `.html`，用户将其完整内容粘贴到对应正则的“替换内容”。内部源码可以拆分 HTML/CSS/JS，但不得把拆分源码当作前端交付物。
...
当前前端表面按以下顺序多轮推进，每轮仍遵循“问题与信息采集 + 推荐方向 → 用户选定或放权 → 给出片段/实现 → 阶段总结”。需求访谈完整规则见 `ui-requirements-interview.md`；每轮最多 3–4 个问题，只问当前表面和当前阶段的缺口：

1. 从已锁定 RP 与运行数据中整理已知事实，建立当前表面的 `ui_requirements`；
2. 锁定用途、主要旅程、等级、设备优先级和体验载体；
3. 整理字段候选、信息优先级、空态/错误态、数据规模与组件建议；
4. 锁定交互、宿主动作、反馈、回退和可撤销/重试行为；
5. 给出 2–4 个项目专属视觉方向，锁定配色、密度、主题与动效；
6. 按 `ui-assets.md` 决定字体、图标、内嵌 SVG、远程资源和回退策略；
7. 写前端产品方案：信息架构、页面地图、主要旅程、数据来源、宿主动作；
8. 写数据契约：可见字段 → 真实路径 → 刷新时机 → 缺失/错误表现；
9. 建应用目录与 `ui-app.yaml`；
10. 只做 HTML 结构与语义，不急着内联；
11. 独立完成设计令牌、布局、组件、主题演出和响应式；
12. 用模拟状态完成普通浏览器满数据、空态、错误态、长列表、长中文和字体/图标回退验收；
13. 实现交互和操作反馈；
14. 通过 `host-adapter.js` 接入真实 SillyTavern/MVU/Tavern Helper；
15. 在真实宿主中检查数据、按钮、编辑、Swipe、重载、聊天切换和 Blob URL 模式；
16. 执行 `ui-build`，生成自包含 HTML；
17. 正则与项目清单只引用最终完整 HTML，最后生成分开的正则 JSON 和 `.html` 交付组件。
...
- 正则配置与构建结果配对，交付 HTML 不引用开发目录；用户按说明将 HTML 全文粘贴到对应 `replaceString`；
- 构建角色卡前不存在 `ui.app_stale`；
- 默认交付是多文件 RP 项目包；前端 HTML、正则配置和其他组件按项目清单配对，不提供单文件角色卡作为替代交付路线。
