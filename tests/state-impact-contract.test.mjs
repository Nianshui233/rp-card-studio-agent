...
const runtimeSkillPath = path.join(root, 'internal-skills', 'st-runtime-authoring', 'SKILL.md');
...
test('late frontend requests must classify state ownership before implementation', () => {
  ...
  assert(coverage().stages.mvu_ejs.decisions.some(decision => decision.id === 'state-ownership'), 'runtime stage must own canonical state extension decisions');
...
test('cross-stage state changes route back to the owning stage and forbid hidden frontend state', () => {
  ...
  const runtime = read(runtimeSkillPath);
  ...
  assert.match(controller, /mvu_ejs/);
  ...
  assert.match(runtime, /唯一状态根|状态.*所有权|派生/);
});

test('coverage declares the state-owning stages that a late UI request can reopen', () => {
  const data = coverage();
  for (const name of ['systems', 'scenes', 'character', 'mvu_ejs']) {
  ...
  assert.equal(data.reopen_routes.message_frontend_new_persistent_state, 'mvu_ejs');
  assert.deepEqual(data.reopen_routes.message_frontend_new_character_attribute_semantics, ['character', 'mvu_ejs']);
  assert.deepEqual(data.reopen_routes.message_frontend_new_gameplay_mechanic, ['systems', 'mvu_ejs']);
