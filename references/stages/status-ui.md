...
- 若宿主插入了 iframe 元素但其子文档没有导航或脚本未执行，状态栏没有运行。发现 MVU 初始化缺失或 iframe 空白时，先打开 `酒馆助手 -> 渲染`，关闭 `启用 Blob URL 渲染`，刷新 SillyTavern 后重新进入目标聊天；开启状态下记录 blocker `tavern_helper_blob_url_rendering`，不得仅凭 iframe 元素、Blob 内容或静态截图判定通过。
