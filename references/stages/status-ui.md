...
| 问题 | 方向 | 影响 | 推荐 |
|---|---|---|---|
| 状态栏怎样进入消息？ | SillyTavern 角色正则 / Tavern Helper 消息级 JS | 影响复杂度和宿主依赖 | 推荐角色正则，足以覆盖大多数文字与 HTML 状态栏 |
...
| 历史逐楼层快照 | 不保证；普通宏可能读取最近变量消息 | 可按 iframe 自身数字楼层 ID 读取并验收 |
...
- `adapter: tavern_helper_message` 用于复杂消息级交互、动态状态切换，或明确要求重载历史后仍读取各楼层旧快照的项目。它必须使用 `level: host_required`，从消息自己的 iframe 取得数字楼层 ID，再按该 ID 读取变量；未提供并实测实现时不得冒充 `embedded` 成品。
...
1. Prompt 过滤：从发给模型的历史副本中移除完整或流式未闭合的 `<UpdateVariable>` / `<update>` 块，不修改原始聊天记录。
2. 流式显示：生成未完成时把更新块折叠为“变量更新中”，保留更新块之后的状态栏占位符。
3. 完整显示：生成完成后把变量更新内容折叠为可查看的记录。
4. 状态栏投影：最后把消息占位符替换成状态栏。
...
- 消息删除、切聊或加载历史：每条消息只拥有自己的投影或 iframe；不得留下跨消息、跨聊天的监听器或状态缓存。
- 若项目要求逐楼层历史快照：加载至少两个状态不同的历史楼层，确认消息级 iframe 分别取得自身数字 ID 并读取对应变量；纯角色正则加 DOM 宏不满足这一声明。
...
- `data.extensions.regex_scripts` 包含启用的消息内状态栏规则；启用 MVU 时还包含 Prompt 过滤和显示折叠规则。
...
- 不依赖参考文件或未登记远程页面；运行资源自包含或有明确宿主依赖。
