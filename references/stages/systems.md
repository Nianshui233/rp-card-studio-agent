...
- 动态人物项目按需定义个体生成、去重、稳定身份、跨轮连续性、晋升为长期人物以及离场/回收的业务规则。
...
- 动态人物怎样获得唯一 ID、怎样避免同一身份被重复生成、哪些事实需要跨轮保留，以及何时晋升、离场或回收。
...
- 启用动态人物时，生成、唯一性、身份稳定、连续性和生命周期均有可判定规则；没有把动态个体伪装成预先创作的角色源码。
...
- 路线包含 `scenes`：进入“场景”；若系统结果显示场景不再适用，登记返回预检调整。
- 路线跳过 `scenes`：按 `workflow.selected_stages` 进入 `mvu_ejs` 或直接进入 `narrative_opening`，不得在系统阶段重新选择路线。
- 系统无跨轮状态需求：可以回退并把本阶段标为 `skipped`，不要为了形式保留无消费者变量。
- 发现变量依赖尚未定义的角色行为或世界规则：返回对应阶段做最小补丁，再恢复系统阶段。

阶段完成后按 `workflow.selected_stages` 自动进入下一个已选阶段，不再询问是否启用场景或其他可选阶段。
