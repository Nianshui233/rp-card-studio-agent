...
- 状态栏与配套 UI 只通过角色正则和 Tavern Helper 消息级 JS/iframe 交付。`basic_status` 兼容路线使用 `embedded + sillytavern_regex`，固定为 `refresh: on_message`、只读、无命令、无 tabs 的文字或简单静态 HTML，且不保证重载历史快照。完整 `light`/`medium`/`heavy` UI 必须装配已锁定的 `ui_experience`、`ui_theme`、`ui_bindings` 与多个 `ui_component`，逐组件生成稳定 UUID 的中文命名角色正则；模型生产的 block marker 同时生成中文 CharacterBook 输出协议。完整 UI 固定为 `tavern_helper_message + host_required`，由角色正则生成自包含 fenced HTML，再由 Tavern Helper 在消息内创建 iframe。组件可通过 Tavern Helper API、斜杠命令或直接父页面/插件 DOM 完成输入框、发送按钮和其他宿主联动，不要求先寻找替代接口；真实验收需覆盖严格整数 `getCurrentMessageId()`、同楼 `getChatMessages()`/`getVariables()`、一次性交互绑定，以及宿主监听器/临时节点和 MVU 监听的卸载清理。失败时不得用 latest 冒充历史快照。任何消息 UI 都不得创建页面级常驻状态栏/面板、访问凭据或私密存储、使用危险动态 HTML sink、失控网络/远程 UI，或包含 `$&`/`$1` 等正则替换令牌；也不得要求修改 SillyTavern 本体。没有真实宿主证据时必须保持 `runtime: not_run`。
