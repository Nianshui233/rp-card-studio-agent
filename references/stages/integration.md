...
- 状态栏与配套 UI 只通过角色正则和 Tavern Helper 消息级 JS/iframe 交付。`basic_status` 兼容路线使用 `embedded + sillytavern_regex`，固定为 `refresh: on_message`、只读、无命令、无 tabs 的文字或简单静态 HTML，且不保证重载历史快照。完整 `light`/`medium`/`heavy` UI 必须装配已锁定的 `ui_experience`、`ui_theme`、`ui_bindings` 与少量代表完整页面或真正独立触发块的 `ui_component`，逐页生成稳定 UUID 的中文命名角色正则；共享触发与状态快照的任务、物品、关系、地图等功能成为综合页面内的模块。内部模块按玩家任务映射到不超过 5 个一级入口，不能把 Schema 分组或变量树直接变成导航。模型生产的 block marker 同时生成中文 CharacterBook 输出协议和 Forge 受控捕获组。完整 UI 固定为 `tavern_helper_message + host_required`，由角色正则生成包含 CSS、主体结构和 JS 的自包含 fenced HTML，再由 Tavern Helper 在消息内创建 iframe。状态页等待 `Mvu` 并通过消息 iframe 的 `getAllVariables()` 读取截至当前楼的合并 `stat_data`；模型 block 直接解析捕获负载，不再重读已替换消息。页面可通过 Tavern Helper API、斜杠命令或直接父页面/插件 DOM 完成输入框、发送按钮和其他宿主联动，不要求先寻找替代接口。每个 iframe 按可重复执行设计：重建前清理上一份受管运行实例，静态控件确定性重绑，不用 `pagehide` 的 AbortSignal 杀死本地按钮；MVU/宿主全局监听在实例替换或真正卸载时清理。inline JavaScript 的 `&&`、`||` 后保留空格并扫描命名实体风险，最终替换脚本还需语法检查。真实验收需覆盖不同历史楼层快照、opening swipe、消息编辑、重新生成、iframe 二次执行、切聊/刷新，以及宿主监听器/临时节点和 MVU 监听的卸载清理。失败时不得用 latest 冒充历史快照。任何消息 UI 都不得创建页面级常驻状态栏/面板、访问凭据或私密存储、使用危险动态 HTML sink 或失控网络/远程 UI。用户 inline 源码不得包含 `- 状态栏与配套 UI 只通过角色正则和 Tavern Helper 消息级 JS/iframe 交付。`basic_status` 兼容路线使用 `embedded + sillytavern_regex`，固定为 `refresh: on_message`、只读、无命令、无 tabs 的文字或简单静态 HTML，且不保证重载历史快照。完整 `light`/`medium`/`heavy` UI 必须装配已锁定的 `ui_experience`、`ui_theme`、`ui_bindings` 与少量代表完整页面或真正独立触发块的 `ui_component`，逐页生成稳定 UUID 的中文命名角色正则；共享触发与状态快照的任务、物品、关系、地图等功能成为综合页面内的模块。模型生产的 block marker 同时生成中文 CharacterBook 输出协议和 Forge 受控捕获组。完整 UI 固定为 `tavern_helper_message + host_required`，由角色正则生成包含 CSS、主体结构和 JS 的自包含 fenced HTML，再由 Tavern Helper 在消息内创建 iframe。状态页等待 `Mvu` 并通过消息 iframe 的 `getAllVariables()` 读取截至当前楼的合并 `stat_data`；模型 block 直接解析捕获负载，不再重读已替换消息。页面可通过 Tavern Helper API、斜杠命令或直接父页面/插件 DOM 完成输入框、发送按钮和其他宿主联动，不要求先寻找替代接口；真实验收需覆盖不同历史楼层快照、一次性交互绑定，以及宿主监听器/临时节点和 MVU 监听的卸载清理。失败时不得用 latest 冒充历史快照。任何消息 UI 都不得创建页面级常驻状态栏/面板、访问凭据或私密存储、使用危险动态 HTML sink 或失控网络/远程 UI。用户 inline 源码不得包含 `$&`/`$1` 等替换令牌；Forge 自动生成的受控 `$1` 捕获槽除外。也不得要求修改 SillyTavern 本体。没有真实宿主证据时必须保持 `runtime: not_run`。`/`$1` 等替换令牌；Forge 自动生成的受控 `$1` 捕获槽除外。也不得要求修改 SillyTavern 本体。没有真实宿主证据时必须保持 `runtime: not_run`。
