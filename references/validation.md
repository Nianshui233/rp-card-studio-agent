...
- 轻/中/重 UI 通过拆分大量同壳独立正则凑等级，而综合工作区缺少 4/8/12 个具有真实绑定的页内模块；把内部模块直接展开成超过 5 个玩家一级入口；或模型 block 正则没有捕获并传入当轮负载，状态页没有等待 `Mvu`/读取 `getAllVariables()`，导致只渲染空壳。（状态栏/UI、整合）
- 玩家可见 UI 直接暴露技术 ID、变量键、未知英文枚举、`uninitialized`、空对象/数组，使用通用递归对象查看器，或存在重复事实、巨大 Grid 空白、横向溢出和一次展开超长集合等明显玩家体验失败。（状态栏/UI、整合）
- UI 内联源码创建或恢复页面级常驻状态栏/面板，访问凭据或私密存储，使用动态代码、危险 HTML sink、失控网络/远程 UI，含 `- 轻/中/重 UI 通过拆分大量同壳独立正则凑等级，而综合工作区缺少 4/8/12 个具有真实绑定的页内模块；或模型 block 正则没有捕获并传入当轮负载，状态页没有等待 `Mvu`/读取 `getAllVariables()`，导致只渲染空壳。（状态栏/UI、整合）
- UI 内联源码创建或恢复页面级常驻状态栏/面板，访问凭据或私密存储，使用动态代码、危险 HTML sink、失控网络/远程 UI，含 `$&`/`$1` 等 SillyTavern 替换令牌，重复绑定交互，或未移除 MVU/宿主事件监听与临时节点。（状态栏/UI、整合）`/`$1` 等 SillyTavern 替换令牌，含可能被 HTML 命名实体改写的 `&&变量`/`||变量`，重复绑定交互，用 `pagehide` AbortSignal 杀死本地控件，或未移除 MVU/宿主事件监听与临时节点。（状态栏/UI、整合）
...
9. 从中国玩家视角检查桌面和 375px：无技术 ID、英文机器键、未知枚举、重复日期/地点/旅行状态、巨大空白、横向溢出或一次展开的超长列表；按钮至少 44px，键盘焦点可见，人物/地点/情报/事务等专用视图信息顺序合理。
