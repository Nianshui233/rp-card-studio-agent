...
- With MVU, generate separate prompt/display regexes that hide only complete `<initvar>...</initvar>` blocks and preserve the raw message, a prompt-only update filter that keeps the newest two depths by default, and stream/final display hiding for update blocks. Generate `[不发送]界面占位符` separately from `[界面]状态栏`. Use stable UUIDs, non-greedy multi-block matching, fixed managed order, and collision protection.
