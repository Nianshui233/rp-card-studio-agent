...
- Full `light`, `medium`, and `heavy` UI uses UI 2.0 sources: one `ui_experience`, one referenced `ui_theme`, one referenced `ui_bindings`, and one `ui_component` per selected surface. Forge compiles every component into a stable UUID character regex with a Chinese display name. Model-produced block markers also compile into dedicated Chinese-named CharacterBook output contracts. The card still builds to one `.json` by default.
- `light` requires at least six real components with an entry, `status_terminal`, and a narrative component. `medium` requires at least twelve, a setup surface, four dashboards/workspaces, and two narrative components. `heavy` requires at least twenty-four, eight dashboards/workspaces, four narrative components, and `hub` or `mixed` navigation. Duplicate or empty panels do not satisfy these capability floors.
...
- Full UI components require `tavern_helper_message + host_required`. Each regex emits self-contained fenced HTML for the same message iframe. The runtime validates integer `getCurrentMessageId()`, reads `getChatMessages(message_id)` and `getVariables({ type: "message", message_id })` only for that floor, uses documented message/slash/swipe/MVU event interfaces, binds static interactions once, and removes the exact MVU listener on `pagehide`/`unload`.
- Inline UI sources must not contain parent-page access, storage, `eval`/`new Function`, unsafe HTML sinks, inline event handlers, network requests, unregistered remote resources, host-fragile interactions without authorization, or SillyTavern replacement tokens such as `$&`, `$1`, `$`` and `$'`. Dynamic values use `textContent` or controlled DOM construction. No component may read `latest`, create page-level nodes, or require a modification to SillyTavern or Tavern Helper.
